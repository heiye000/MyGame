...
## 打开某动作的精确输入判定窗口。
func open(action_type: PlayerActionType.ActionType, window_frames: int) -> void:
	_open_until[action_type] = Engine.get_physics_frames() + window_frames


## 关闭窗口并清掉相关预按状态。
func close(action_type: PlayerActionType.ActionType) -> void:
	_open_until.erase(action_type)
	_pre_input_until.erase(action_type)


## 这个动作的判定窗口是否还开着。
func is_open(action_type: PlayerActionType.ActionType) -> bool:
	return get_remaining_window_frames(action_type) > 0


## 窗口还剩几物理帧，没有则返回 -1。
func get_remaining_window_frames(action_type: PlayerActionType.ActionType) -> int:
...
## 宽松模式：记下窗口打开前提前按下的键。
func capture_pre_input(action_type: PlayerActionType.ActionType, pre_buffer_frames: int) -> void:
	_pre_input_until[action_type] = Engine.get_physics_frames() + pre_buffer_frames


## 是否还有未过期的提前按键。
func has_pre_input(action_type: PlayerActionType.ActionType) -> bool:
	return get_pre_input_remaining_frames(action_type) > 0


## 提前按键还剩几物理帧，没有则返回 -1。
func get_pre_input_remaining_frames(action_type: PlayerActionType.ActionType) -> int:
...
## 用掉提前按键，成功返回 true。
func consume_pre_input(action_type: PlayerActionType.ActionType) -> bool:

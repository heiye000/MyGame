...
4. **同帧门闩（防 looped transitions）**：`_physics_process` 开头记下 `playback.get_current_node()` 到 `_root_node_at_frame_start`；`is_*()` 若本帧开始已在对应 Oneshot 子机（如 `AttackMachine`）则返回 `false`。否则 Oneshot `At End` → Move 同帧缓冲仍为 true 会立刻再进，触发 `looped transitions in a single frame` 告警并可能中止过渡。连招/连滚因此晚 1 物理帧，属预期。
5. **Profile Entry 用独立脚本**；`.tres` 的 `entries` 写成 `[SubResource(...), ...]`，禁止 `Array[ExtResource]`。
6. **开招朝向**：进入子机瞬间优先读当前 WASD；无输入再回退 `last_direction`；整段动作锁定，且只写本机 blend（不写 Move idle/run）。
...
5. Step1/2：生成动画与树。
6. `PlayerAnimationTree`：加 `is_*()`（triggered OR has_buffered；并对新子机名做同帧门闩）。
...
表达式在 `advance_expression_base_node`（设为 `NodePath(".")`，即 **AnimationTree 自身**）上求值，故 `get_move_direction()`、`is_attacking()`、`is_rolling()` 必须是 `PlayerAnimationTree` 成员。`is_attacking()` / `is_rolling()` 只查 `is_triggered()` 与 `has_buffered()`，**不要**在表达式内 `consume_buffered()`；并须带同帧门闩（见上方「必须遵守」第 4 条），否则连续攻击/翻滚会触发 `looped transitions` 告警。
...
2. 重跑 Step1/Step2。
3. `PlayerAnimationTree` 增加 `is_skill()`（只查 `has_buffered`，不消费；对 `SkillMachine` 做同帧门闩）；模式态进入时 `consume_buffered()`，并补 `match` 与 `_set_*_blend`。

## godot-sprite-anim-pipeline / Step 3 模板：探索态
## 基准：GamePlayer/actors/player/state_machine/normal.gd。挂到 LimboHSM/Normal。
## 只写 Normal 子图的八向移动。拔剑时 dispatch draw_sword，过渡由本态在 _setup 注册。
## 探索期间清掉攻击/翻滚缓冲，避免拔剑后立刻出招。blend 用 Direction8.to_blend_position，不要再手写 y 取反。

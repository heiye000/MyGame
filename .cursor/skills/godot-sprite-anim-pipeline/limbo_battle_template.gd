## godot-sprite-anim-pipeline / Step 3 模板：战斗态
## 基准：GamePlayer/actors/player/state_machine/battle.gd。挂到 LimboHSM/Battle。
## 位移八向；身体动画只折成 down_left / down_right。刀光用折之前的锁定瞄准，在 AttackMachine 进入 active 时挥一次。
## 不要在进入 AttackMachine（windup）时挥刀。不要给角色攻击动画再写 Hitbox 多边形。
...
func _process_attack_machine(player: Player, move_direction: Vector2) -> void:
	...
	if phase == &"active" and not _slash_sent:
		_slash_sent = true
		# 仅 ATTACK_L：已经指定 attack_left_1。新动作没点名刀光时不要抄这三行。

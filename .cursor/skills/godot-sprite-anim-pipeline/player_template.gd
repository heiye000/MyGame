...
## 基准：GamePlayer/actors/player/player.gd。根节点就是 CharacterBody2D，不要再套一层 Node2D。
## 职责：节点引用、AnimationTree 物理帧对齐、初始化 LimboHSM、is_battle_mode()。
## 不写行动 match、不消费缓冲、不挥刀、不查锁定。那些在 limbo_normal_template / limbo_battle_template。
## 保留 attack_caster、target_lock、hurtbox、hitbox。Step3 不得把这些引用删掉。
